...
System.out.println(name.toUpperCase());
		...
		//    HINT: Use .length() to determine the number of characters in the String.
				...
				//    use .charAt()
for(int character = 0; character <= name.length();character +=1) {	

				// 5. Use MODULO operator (%) to identify if it is an EVEN or ODD character.
			if(character % 2 == 0) {
				System.out.println(name.toUpperCase().charAt(character));
			}
			else {
				System.out.println(name.toLowerCase().charAt(character));
			}
